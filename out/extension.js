...
const acme_formatter_1 = require("./acme-formatter");
function loadInstructionData(context) {
    try {
        ...
        acme_formatter_1.AcmeFormatter.loadInstructionsFromJson(instructions);
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.languages.registerDocumentFormattingEditProvider('acme', formattingProvider));
    // Register document range formatting provider
    context.subscriptions.push(vscode.languages.registerDocumentRangeFormattingEditProvider('acme', formattingProvider));
    // Register hover provider for instruction documentation
    const hoverProvider = new AcmeHoverProvider(context);
    context.subscriptions.push(vscode.languages.registerHoverProvider('acme', hoverProvider));
    // Register ACME assembler commands
    const acmeAssembler = new AcmeAssembler();
    context.subscriptions.push(vscode.commands.registerCommand('acme.assemble', (uri) => {
        acmeAssembler.assembleFile(uri);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('acme.assembleAndRun', (uri) => {
        acmeAssembler.assembleAndRun(uri);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('acme.assembleAndRunEmulator', (uri) => {
        acmeAssembler.assembleAndRunEmulator(uri);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('acme.assembleAndRunMega65', (uri) => {
        acmeAssembler.assembleAndRunMega65(uri);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('acme.buildProject', (uri) => {
        acmeAssembler.buildProject(uri);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('acme.showOutput', () => {
        acmeAssembler.showOutput();
    }));
    // Register task provider
    context.subscriptions.push(vscode.tasks.registerTaskProvider('acme', new AcmeTaskProvider()));
...
class AcmeFormattingProvider {
    constructor() {
        this.formatter = new acme_formatter_1.AcmeFormatter();
    }
    ...
    formatDocument(document, options, token, range) {
        ...
        if (formattedText !== text) {
            const fullRange = range || new vscode.Range(document.positionAt(0), document.positionAt(text.length));
...
class AcmeAssembler {
    ...
    shouldShowTerminalOnRun() {
        const config = this.getConfig();
        return config.get('showTerminalOnRun', true);
    }
    async hasToDirective(filePath) {
        try {
    ...
    async getToDirectiveOutput(filePath) {
        try {
            const content = await fs.promises.readFile(filePath, 'utf8');
    ...
    async assembleAndRunEmulator(uri) {
        await this.assembleFile(uri);
        const fileUri = uri || vscode.window.activeTextEditor?.document.uri;
        if (!fileUri)
            return;
    ...
    async assembleAndRunMega65(uri) {
        await this.assembleFile(uri);
        const fileUri = uri || vscode.window.activeTextEditor?.document.uri;
        if (!fileUri)
            return;
    ...
    async runWithEtherload(prgFile) {
    ...
    async runAcme(inputFile, outputFile, workingDir) {
    ...
    parseAndShowErrors(output, inputFile) {
        const diagnostics = [];
        ...
        for (const line of lines) {
            ...
            if (errorMatch) {
                ...
                const diagnostic = new vscode.Diagnostic(new vscode.Range(lineNumber, 0, lineNumber, Number.MAX_VALUE), message, vscode.DiagnosticSeverity.Error);
            ...
            if (warningMatch) {
                ...
                const lineNumber = parseInt(lineStr) - 1; // Convert to 0-based index
                const diagnostic = new vscode.Diagnostic(new vscode.Range(lineNumber, 0, lineNumber, Number.MAX_VALUE), message, vscode.DiagnosticSeverity.Warning);
                diagnostic.source = 'ACME';
                diagnostics.push(diagnostic);
            }
        }
    ...
    clearErrorsForFile(inputFile) {
        const uri = vscode.Uri.file(inputFile);
        this.diagnosticsCollection.set(uri, []);
    }
}
class AcmeTaskProvider {
    provideTasks(token) {
        const tasks = [];
        if (vscode.workspace.workspaceFolders) {
            for (const folder of vscode.workspace.workspaceFolders) {
                ...
                vscode.workspace.findFiles('**/*.{a,asm}', '**/node_modules/**', undefined, token)
                    .then(files => {
                    files.forEach(file => {
                        const task = new vscode.Task({ type: 'acme' }, folder, `Assemble ${path.basename(file.fsPath)}`, 'ACME', new vscode.ShellExecution('acme', ['-o', '${workspaceFolder}/out/${fileBasenameNoExtension}.prg', '${file}']), 'acme');
...
class AcmeHoverProvider {
    ...
    createHoverContent(instruction) {
        ...
        if (instruction.addressingModes && instruction.addressingModes.length > 0) {
            ...
            for (const mode of instruction.addressingModes) {
                ...
                cycleModifiers.forEach((mod) => usedModifiers.add(mod));
