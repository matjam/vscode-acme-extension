...
export class AcmeFormatter {
    ...
    private static readonly CONDITIONAL_DIRECTIVES = [
        '!if', '!ifdef', '!ifndef', '!else', '!endif'
    ];
    formatDocument(document: string, options: FormattingOptions): string {
    ...
    formatLine(line: string, options: FormattingOptions, indentLevel: number, indentString: string): string {
        const trimmed = line.trim();
    ...
    formatBlock(block: string[], options: FormattingOptions, indentLevel: number, indentString: string): string[] {
    ...
    calculateNextIndentLevel(line: string, currentLevel: number): number {
        const trimmed = line.trim();
    ...
    formatDirective(line: string, options: FormattingOptions): string {
    ...
    formatDataDefinition(line: string, options: FormattingOptions): string {
    ...
    formatInstruction(line: string, options: FormattingOptions): string {
    ...
    formatInstructionOnly(instruction: string, options: FormattingOptions): string {
    ...
    formatSymbolDefinition(line: string, options: FormattingOptions): string {
    ...
    formatPcDefinition(line: string, options: FormattingOptions): string {
    ...
    formatComment(line: string): string {
    ...
    isColonLabel(line: string): boolean {
        const trimmed = line.trim();
    ...
    isInlineLabel(line: string): boolean {
        const trimmed = line.trim();
    ...
    isAnonymousLabel(line: string): boolean {
        const trimmed = line.trim();
        // Check for anonymous forward/backward labels (+, ++, +++, -, --, ---, etc.)
        return /^[+\-]+$/.test(trimmed);
    }

    hasBranchToAnonymousLabel(line: string): boolean {
        const trimmed = line.trim();
    ...
    isOpeningBrace(line: string): boolean {
        const trimmed = line.trim();
        return trimmed === '{';
    }

    isClosingBrace(line: string): boolean {
        const trimmed = line.trim();
        // Only consider it a closing brace if it's just '}' or '} else' (without opening brace)
        return trimmed === '}' || (trimmed.startsWith('}') && !trimmed.includes('{'));
    }

    hasOpeningBrace(line: string): boolean {
        const trimmed = line.trim();
        return trimmed.includes('{');
    }

    hasInlineBraces(line: string): boolean {
        const trimmed = line.trim();
    ...
    splitInlineBraces(line: string, indentLevel: number, indentString: string): string[] {
        const trimmed = line.trim();
        const result: string[] = [];
